...
import { Bell, X, CheckCircle2, AlertTriangle, Info, AlertCircle,
  ...
  LayoutGrid, List, Search } from 'lucide-react';
...
const colorMap: Record<string, { icon: string; bg: string; border: string; badge: string }> = {
  ...
  BUS_ARRIVE:              { icon: 'text-emerald-600',bg: 'bg-emerald-50 dark:bg-emerald-500/10',border: 'border-emerald-200 dark:border-emerald-500/20',badge: 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400' },
};
...
function NotificationCard({ n, index }: { n: NotificationDTO; index: number }) {
  ...
  return (
    ...
    >
      <div className={`h-1 ${c.badge.split(' ').find((cl: string) => cl.startsWith('bg-')) || 'bg-emerald-500'}`} />
      <div className="p-5">
            ...
            {info.matricule && (
              <div className="bg-slate-50 dark:bg-zinc-800 rounded-xl p-3 mb-3 border border-slate-100 dark:border-zinc-700">
                <div className="flex items-center gap-2 text-sm font-black text-slate-800 dark:text-white mb-2">
                  <Bus size={13} className="text-emerald-500" />
                  {info.matricule as string}
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs text-slate-500 dark:text-zinc-400">
                  {info.compagnie && (
                    <span className="flex items-center gap-1.5"><MapPin size={11} className="text-emerald-500" />Compagnie : <span className="font-bold text-slate-700 dark:text-zinc-300 ml-0.5">{info.compagnie as string}</span></span>
                  )}
                  {info.quai && (
                    <span className="flex items-center gap-1.5"><MapPin size={11} className="text-emerald-500" />Quai : <span className="font-bold text-slate-700 dark:text-zinc-300 ml-0.5">{info.quai as string}</span></span>
                  )}
                  {info.stationnementId && (
                    <span className="flex items-center gap-1.5"><Clock size={11} className="text-emerald-500" />Stationnement #<span className="font-bold text-slate-700 dark:text-zinc-300 ml-0.5">{info.stationnementId as number}</span></span>
                  )}
                </div>
              </div>
            )}
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 40%, #0d9488 100%)' }}
      >
        ...
        <div className="relative flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              {pendingCount > 0 && (
                <span className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-white text-emerald-600 text-[10px] font-black rounded-full flex items-center justify-center border-2 border-emerald-500 shadow-md animate-bounce">
      ...
      <AnimatePresence>
        {pendingCount > 0 && !syncing && (
          <motion.div
            ...
            className="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-500/10 dark:to-teal-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-2xl p-4 flex items-center justify-between"
          >
            ...
            <button onClick={sync}
              className="text-xs font-black text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-500/20 px-4 py-2 rounded-xl hover:bg-emerald-200 dark:hover:bg-emerald-500/30 transition">
              Voir maintenant
            </button>
          </motion.div>
        )}
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
        ...
        <div className="relative w-full max-w-xs">
          ...
          <input type="text" placeholder="Rechercher..." value={searchQuery}
            ...
            className="w-full pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-all text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-zinc-500" />
        </div>
        <div className="flex items-center gap-2 ml-auto">
          <div className="flex items-center gap-1 bg-slate-100 dark:bg-zinc-800 p-1.5 rounded-xl">
            <button onClick={() => setViewMode('cards')}
              className={clsx('p-2 rounded-lg transition-all', viewMode === 'cards' ? 'bg-white dark:bg-zinc-700 shadow-sm text-emerald-500' : 'text-slate-400 dark:text-zinc-500 hover:text-slate-600 dark:hover:text-zinc-300')}><LayoutGrid size={18} /></button>
            <button onClick={() => setViewMode('table')}
              className={clsx('p-2 rounded-lg transition-all', viewMode === 'table' ? 'bg-white dark:bg-zinc-700 shadow-sm text-emerald-500' : 'text-slate-400 dark:text-zinc-500 hover:text-slate-600 dark:hover:text-zinc-300')}><List size={18} /></button>
          </div>
        </div>
      </motion.div>
      ...
      {!loaded ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          ...
          <button onClick={loadHistory}
            className="text-sm font-bold text-emerald-500 hover:text-emerald-600 underline underline-offset-2 transition">
                ...
                {filtered.map((n, idx) => {
                  ...
                  return (
                    <motion.tr key={n.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.02 }}
                      className="hover:bg-emerald-50/40 dark:hover:bg-zinc-800/50 transition-colors">
        ...
        <>
          ...
          {notifications.length > 20 && (
            <motion.button
              ...
              className="w-full flex items-center justify-center gap-2 py-3.5 text-sm font-bold text-slate-600 dark:text-zinc-400 hover:text-emerald-500 dark:hover:text-emerald-400 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm hover:border-emerald-200 dark:hover:border-emerald-900 hover:bg-emerald-50/30 dark:hover:bg-zinc-800 transition-all"
            >
